...
fn migrate_server_section(doc: &mut DocumentMut, added_fields: &mut Vec<String>) -> Result<()> {
    ...
    let existing_origins = doc
        .get("security")
        .and_then(|section| section.get("allowed_origins"))
        .cloned();

    let server = doc["server"]
        .as_table_mut()
        .ok_or_else(|| ConfigIoError::InvalidConfig("Invalid [server] section in config".to_string()))?;
    ensure_field(server, "host", Value::from("0.0.0.0"), added_fields);
    ensure_field(server, "port", Value::from(8080), added_fields);
    ensure_field(server, "base_url", Value::from("http://localhost:8080"), added_fields);
    ensure_field(server, "base_path", Value::from("updater"), added_fields);
    ensure_field(server, "tcp_nodelay", Value::from(true), added_fields);
    ensure_field(server, "timeout_secs", Value::from(60), added_fields);
    ensure_field(server, "max_body_size_mb", Value::from(100), added_fields);
    ensure_field(server, "streaming_threshold_mb", Value::from(100), added_fields);
...
fn migrate_cache_section(doc: &mut DocumentMut, added_fields: &mut Vec<String>) -> Result<()> {
    ...
    let cache = doc["cache"]
        .as_table_mut()
        .ok_or_else(|| ConfigIoError::InvalidConfig("Invalid [cache] section in config".to_string()))?;
    ...
    ensure_field(cache, "max_memory_cache_gb", Value::from(0), added_fields);
    ensure_field(cache, "checksum_buffer_size", Value::from(8192), added_fields);
    ensure_field(cache, "hash_concurrency", Value::from(100), added_fields);
    ensure_field(cache, "config_reload_channel_size", Value::from(100), added_fields);

    let old_batch_size =
        if let Some(Item::Value(Value::Integer(val))) = cache.get("scan_batch_size") {
            Some(*val.value())
        } else {
            None
        };
    ...
    let batch = cache["batch"].as_table_mut().ok_or_else(|| {
        ConfigIoError::InvalidConfig("Invalid [cache.batch] section in config".to_string())
    })?;
...
fn migrate_hot_reload_section(doc: &mut DocumentMut, added_fields: &mut Vec<String>) -> Result<()> {
    let old_config_debounce = if let Some(cache) = doc.get("cache") {
        cache.get("config_watch_debounce_ms").and_then(|value| value.as_integer())
    } else {
        None
    };

    let old_files_debounce = if let Some(cache) = doc.get("cache") {
        cache.get("file_watcher_debounce_ms").and_then(|value| value.as_integer())
    } else {
        None
    };
    ...
    let hot_reload = doc["hot-reload"].as_table_mut().ok_or_else(|| {
        ConfigIoError::InvalidConfig("Invalid [hot-reload] section in config".to_string())
    })?;
    ...
    let config = hot_reload["config"].as_table_mut().ok_or_else(|| {
        ConfigIoError::InvalidConfig("Invalid [hot-reload.config] section in config".to_string())
    })?;
    ...
    let files = hot_reload["files"].as_table_mut().ok_or_else(|| {
        ConfigIoError::InvalidConfig("Invalid [hot-reload.files] section in config".to_string())
    })?;
    ...
    if let Some(cache) = doc.get_mut("cache").and_then(|section| section.as_table_mut()) {
...
fn migrate_storage_section(doc: &mut DocumentMut, added_fields: &mut Vec<String>) -> Result<()> {
    ...
    let storage = doc["storage"]
        .as_table_mut()
        .ok_or_else(|| ConfigIoError::InvalidConfig("Invalid [storage] section in config".to_string()))?;
    ...
    let s3 = storage["s3"].as_table_mut().ok_or_else(|| {
        ConfigIoError::InvalidConfig("Invalid [storage.s3] section in config".to_string())
    })?;
...
fn migrate_cdn_section(doc: &mut DocumentMut, added_fields: &mut Vec<String>) -> Result<()> {
    ...
    let cdn = doc["cdn"]
        .as_table_mut()
        .ok_or_else(|| ConfigIoError::InvalidConfig("Invalid [cdn] section in config".to_string()))?;
...
fn migrate_cloudflare_section(doc: &mut DocumentMut, added_fields: &mut Vec<String>) -> Result<()> {
    ...
    let cloudflare = doc["cloudflare"].as_table_mut().ok_or_else(|| {
        ConfigIoError::InvalidConfig("Invalid [cloudflare] section in config".to_string())
    })?;
...
fn migrate_servers_array(doc: &mut DocumentMut, added_fields: &mut Vec<String>) -> Result<()> {
    if let Some(servers_array) = doc
        .get_mut("servers")
        .and_then(|servers| servers.as_array_of_tables_mut())
    {
