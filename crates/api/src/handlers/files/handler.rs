...
pub async fn serve_file(
    ...
    // Get config values (supports hot reload)
    let (base_url, base_path, streaming_threshold_bytes) = {
        let config = state.config.read().await;
        (
            config.server.base_url.clone(),
            config.server.base_path.clone(),
            config.server.streaming_threshold_mb * 1024 * 1024,
        )
    };
    ...
    let full_path = FileSystem::build_server_path(&base_path, &server_config.name)
        .join(&actual_path);

    disk::serve_from_disk(full_path, streaming_threshold_bytes).await
}
