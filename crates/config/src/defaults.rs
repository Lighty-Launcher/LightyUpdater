...
pub fn max_memory_cache_gb() -> u64 {
    1  // 1GB default limit (prevents memory leaks, 0 = unlimited)
}

pub fn batch_size_default() -> usize {
    10  // Reduced to 10 for VPS with limited resources
}
...
pub fn checksum_buffer_size() -> usize {
    65536  // 64KB buffer for SHA1 calculation (increased from 8KB to reduce syscalls)
}

pub fn hash_concurrency() -> usize {
    10  // Reduced to 10 for VPS with limited resources (matches batch size)
}
