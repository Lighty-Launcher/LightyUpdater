use crate::bootstrap::{build, initialize, initialize_folders, load};
...
pub fn default_config_path() -> String {
    std::env::var("LIGHTY_CONFIG").unwrap_or_else(|_| "config.toml".to_string())
}

pub async fn run_server_with_default_config(app_version: &str) -> Result<()> {
    let config_path = default_config_path();
    run_server(config_path, app_version).await
}
...
pub async fn run_server(config_path: String, app_version: &str) -> Result<()> {
    ...
    let (app, addr, tcp_nodelay, base_url) = {
        ...
        let app_state = AppState::new(Arc::clone(&cache_manager), Arc::clone(&config));
        let app = build(&config_read, app_state);
        ...
        StorageBackendType::S3 => {
            if !config_read.storage.s3.enabled {
                return Err(RuntimeError::InvalidConfiguration(
                    "S3 backend selected but not enabled in configuration".to_string(),
                ));
            }
...
async fn bind_server(addr: &str) -> Result<tokio::net::TcpListener> {
    tokio::net::TcpListener::bind(addr).await.map_err(|source| {
        if source.kind() == std::io::ErrorKind::AddrInUse {
            ...
            tracing::error!("Failed to bind server on {}: {}", addr, source);
        }
        RuntimeError::BindAddress {
            addr: addr.to_string(),
            source,
        }
    })
