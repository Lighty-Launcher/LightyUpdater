        ...
        while rx.recv().await.is_some() {
            ...
            match load_config_no_migration(config_path).await {
                Ok(new_config) => {
                    ...
                    // Detect removed servers (clean up memory)
                    let removed_servers: Vec<_> =
                        old_servers.difference(&new_servers).cloned().collect();
                    ...
                    *config_write = new_config;
                    ...
                    // Clean up removed servers (prevent memory leaks)
